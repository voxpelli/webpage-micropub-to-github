...
module.exports = function (githubTarget, micropubDocument, siteUrl) {
  ...
  return formatter.formatAll(micropubDocument)
    .then(function (formatted) {
      console.log('Received URL:', formatted.raw.properties.url);
      console.log('Calculated URL:', formatted.url);

      if (formatted.raw.properties.url && formatted.raw.properties.url === formatted.url) {
        console.log('Provided URL matches calculated one');
        force = true;
      }
